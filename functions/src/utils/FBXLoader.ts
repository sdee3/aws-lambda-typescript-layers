...
class FBXLoader extends Loader {
  ...
  parse(FBXBuffer, path) {
    ...
    return new FBXTreeParser(textureLoader, this.manager, fbxTree).parse()
  }
}
...
class FBXTreeParser {
  ...
  manager: any
  fbxTree: any
  constructor(textureLoader, manager, fbxTree) {
    ...
    switch (extension) {
      ...
      case 'tga':
        if (this.manager.getHandler('.tga') === null) {
          console.info('FBXLoader: TGA loader not found, skipping ', fileName)
        }
  ...
  parseModels(skeletons, geometryMap, materialMap) {
    ...
    for (const nodeID in modelNodes) {
      ...
      let model: any = this.buildSkeleton(
        relationships,
        skeletons,
        id,
        node.attrName
  ...
  createCamera(relationships) {
    ...
    if (cameraAttribute === undefined) {
      ...
      switch (type) {
        ...
        default:
          console.info('THREE.FBXLoader: Unknown camera type ' + type + '.')
          model = new Object3D()
          break
      }
    }
...
class TextParser {
  ...
  parseNodeProperty(line, property, contentLine) {
    ...
    const currentNode: any = this.getCurrentNode()
    const parentName: any = currentNode.name
...
class BinaryParser {
  parse(buffer) {
    const reader: any = new BinaryReader(buffer)
...
class BinaryReader {
  ...
  offset: number
  littleEndian: any
  constructor(buffer, littleEndian?: any) {
...
function convertArrayBufferToString(buffer, from?: any, to?: any) {
